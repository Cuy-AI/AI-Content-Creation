...
def test_openai():
    ...
    openai_container = ContainerManager(image="openai:latest", port=8000, use_gpu = False)
    openai_container.start()

    # Create the OpenAI component
    openai_client = openai_container.create_client()

    # Set model name (OpenAI method)
    ...
    # Set schema (OpenAI method)
    answer = openai_client.set_schema(schema=video_script_schema)
    print("Set schema answer", answer)

    answer = openai_client.get_schema()
    print("Get schema answer", answer)

    # Get container output dir:
    saved_output = openai_container.volume_path + "/output/openai/test01/example_output.json" # A sample additional path
    ...
    result = openai_client.generate(prompt=video_prompt, save_path=saved_output)
    print("Generation result:\n", result)

    # Check if output was saved
    saved_path = saved_output
    if not os.path.exists(saved_path):
        raise FileNotFoundError(f"Output json file not found: {saved_path}")

    # Open saved output
    print("Saved json at:", saved_path)
